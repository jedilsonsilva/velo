...
const src = path.join(process.env.TEMP, "swagger-ui-init-mdp.js");
...
for (let p = i; p < txt.length; p++) {
  ...
  if (c === "{") depth++;
    ...
    if (depth === 0) {
      ...
      const outFile = path.join(outDir, "maria-da-penha-openapi.json");
      ...
      console.log("securitySchemes:", JSON.stringify(spec.components?.securitySchemes || spec.securityDefinitions, null, 2));
      ...
      for (const [pName, methods] of Object.entries(spec.paths || {})) {
        for (const method of Object.keys(methods)) {
          if (["get", "post", "put", "patch", "delete", "options", "head"].includes(method)) {
            ...
            console.log(
              `${method.toUpperCase().padEnd(6)} ${pName}  [${(op.tags || []).join(",")}]  ${op.summary || op.operationId || ""}  sec=${JSON.stringify(op.security || spec.security || [])}`
            );
          }
        }
      }
